...
if ($LASTEXITCODE -ne 0) {
    ...
    if ($currentPath -notlike "*$installDir*") {
        ...
        Write-Warning "Run the following command to add $installDir to your PATH so 'droid' is available in new shells:"
        Write-Warning "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path','User') + ';$installDir', 'User')"
    } else {
